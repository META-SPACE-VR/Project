...
public class PlayerRegistry : NetworkBehaviour, INetworkRunnerCallbacks
{
	...
	public static IEnumerable<RoomPlayer> Where(System.Predicate<RoomPlayer> match)
	{
		return Instance.ObjectByRef.Where(kvp => match.Invoke(kvp.Value)).Select(kvp => kvp.Value);
	}

	public static void ForEach(System.Action<RoomPlayer> action)
	{
	...
	public static void ForEachWhere(System.Predicate<RoomPlayer> match, System.Action<RoomPlayer> action)
	{
		foreach (var kvp in Instance.ObjectByRef)
		{
	...
	public static int CountWhere(System.Predicate<RoomPlayer> match)
	{
		int count = 0;
		foreach (var kvp in Instance.ObjectByRef)
		{
	...
	public static bool Any(System.Predicate<RoomPlayer> match)
	{
		foreach (var kvp in Instance.ObjectByRef)
		{
	...
	public static RoomPlayer GetRandom()
	{
		byte index = (byte)Random.Range(0, Count);
		byte i = 0;
		foreach (var kvp in Instance.ObjectByRef)
		{
			RoomPlayer pObj = kvp.Value;
			if (pObj != null)
			{
	...
	public static RoomPlayer[] GetRandom(int count)
	{
